...
def fetch_and_upload_data(**context):
    ...
    try:
        ...
        for table_name in tables:
            try:
                ...
                if not df.empty:
                    ...
                    logger.info(f"Datos de la tabla {table_name} guardados en {file_name}.")
                    ...
                    logger.info(f"Subiendo archivo '{file_name}' al bucket S3 '{bucket_name}' con key '{s3_key}'...")
                    s3_hook.load_file(filename=file_name, key=s3_key, bucket_name=bucket_name, replace=True)
        ...
        logger.error(f"Error al conectar con MySQL o durante el proceso de carga: {err}")
...
def create_status_files(**context):
    ...
    table_status = context['ti'].xcom_pull(key='table_status', task_ids='fetch_and_upload_data')
    ...
    for table_name, status in table_status.items():
        ...
        try:
            ...
            logger.info(f"Archivo de estado '{status_file_key}' subido exitosamente a S3.")
        except Exception as e:
            logger.error(f"Error al subir el archivo de estado '{status_file_path}' a S3: {e}")
            raise RuntimeError(f"Error al subir el archivo de estado '{status_file_path}' a S3: {e}")
        finally:
